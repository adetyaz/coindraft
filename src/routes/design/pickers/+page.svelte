...
<div class="mx-auto max-w-[1360px] px-7 pt-7 pb-18">
	<div class="mb-6.5">
		...
		<p class="mt-2 max-w-[70ch] text-sm text-text-muted">
	...
	<div class="flex flex-col gap-5">
		...
		<div class="rounded-[24px] border border-border bg-surface p-7">
			<div class="mb-5 flex flex-wrap items-baseline gap-3.5">
				<span class="rounded-full bg-primary px-2.5 py-1 font-mono text-xs font-bold text-text">A</span>
				<span class="text-xl font-black tracking-[-0.02em]">The reel rack</span>
				<span class="text-[13px] text-text-muted">Five reels, one per sector. Spin until the pick you want lands in the window.</span>
			</div>
			<div class="grid grid-cols-5 gap-3.5 max-lg:grid-cols-2 max-sm:grid-cols-1">
				{#each SECTORS as s, i (s.name)}
					...
					>
						<div class="py-2.5 text-center text-[10px] font-extrabold tracking-[0.12em] text-text uppercase" style="background:{s.color}">
							{s.name}
						</div>
						<div class="p-2.5">
							<div class="py-1.5 text-center font-mono text-xs text-text-muted">{s.tokens[(idx + n - 1) % n].t}</div>
							<div class="flex flex-col items-center gap-1.5 rounded-xl p-4" style="background:var(--color-surface-alt);border:1px solid {s.color}55">
								<div class="text-xl font-black tracking-[-0.02em]">{cur.t}</div>
								<div class="font-mono text-[11px] text-text-muted">{cur.p}</div>
							</div>
							<div class="py-1.5 text-center font-mono text-xs text-text-muted">{s.tokens[(idx + 1) % n].t}</div>
							<div class="mt-1 text-center font-mono text-[10px] tracking-[0.1em] text-text-muted uppercase">Click to spin</div>
						</div>
					</button>
				{/each}
			</div>
		...
		<div class="rounded-[24px] border border-border bg-surface p-7">
			<div class="mb-2 flex flex-wrap items-baseline gap-3.5">
				<span class="rounded-full bg-primary px-2.5 py-1 font-mono text-xs font-bold text-text">B</span>
				<span class="text-xl font-black tracking-[-0.02em]">The hand</span>
				<span class="text-[13px] text-text-muted">A fanned hand of five. Flip a card to commit that sector.</span>
			</div>
			<div class="flex flex-wrap items-center justify-center gap-2 py-5">
				{#each SECTORS as s, i (s.name)}
					...
					<button
						...
						style="background:var(--color-surface);border:2px solid {up ? s.color : 'var(--color-border-strong)'}"
					>
						{#if up}
							<span class="rounded-full px-2.5 py-1 text-[10px] font-extrabold tracking-[0.1em] uppercase" style="background:{s.color}24;color:{s.color}">{s.name}</span>
							<div class="text-2xl font-black tracking-[-0.02em]">{cur.t}</div>
							<div class="font-mono text-[11px] text-text-muted">{cur.p}</div>
						{:else}
							<span class="rounded-full bg-surface-alt px-2.5 py-1 text-[10px] font-extrabold tracking-[0.1em] text-text-muted uppercase">Closed</span>
		...
		<div class="overflow-hidden rounded-[24px] border border-border bg-surface p-7">
			<div class="mb-5 flex flex-wrap items-baseline gap-3.5">
				<span class="rounded-full bg-primary px-2.5 py-1 font-mono text-xs font-bold text-text">C</span>
				<span class="text-xl font-black tracking-[-0.02em]">The belt</span>
				<span class="text-[13px] text-text-muted">The market runs past on a conveyor. Grab what you want before it leaves the window.</span>
			</div>
			<div class="mb-4.5 grid grid-cols-5 gap-3 max-lg:grid-cols-2 max-sm:grid-cols-1">
				{#each SECTORS as s, i (s.name)}
					{@const filled = i === 0 || i === 2}
					<div class="rounded-2xl border p-4 text-center" style="border-color:{filled ? s.color : 'var(--color-border-strong)'}">
						<div class="text-[10px] font-extrabold tracking-[0.1em] uppercase" style="color:{s.color}">{s.name}</div>
						<div class="mt-2.5 text-lg font-black" style={filled ? '' : 'color:var(--color-text-muted)'}>{filled ? s.tokens[0].t : 'waiting'}</div>
					</div>
				{/each}
			</div>
			<div class="anim-marquee flex w-max gap-3 border-y-2 border-dashed border-border py-4">
				{#each [...SECTORS, ...SECTORS] as s, repIdx (repIdx)}
					{#each s.tokens as t (t.t)}
						<div class="flex shrink-0 items-center gap-2.5 rounded-full border border-border bg-surface px-4 py-2.5">
		...
		<div class="rounded-[24px] border border-border bg-surface p-7">
			<div class="mb-5 flex flex-wrap items-baseline gap-3.5">
				<span class="rounded-full bg-primary px-2.5 py-1 font-mono text-xs font-bold text-text">D</span>
				<span class="text-xl font-black tracking-[-0.02em]">The claim board</span>
				<span class="text-[13px] text-text-muted">Every token is a cell. Claim one cell per sector column.</span>
			</div>
			<div class="grid grid-cols-5 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1">
				{#each SECTORS as s, i (s.name)}
					<div>
						<div class="mb-3 text-center text-[10px] font-extrabold tracking-[0.1em] uppercase" style="color:{s.color}">{s.name}</div>
						<div class="flex flex-col items-center gap-2">
							{#each s.tokens as tk, j (tk.t)}
								...
								<button
									...
									style={on ? `background:${s.color};color:var(--color-ink)` : 'background:var(--color-surface-alt);color:var(--color-text-muted)'}
								>
									{tk.t}
								</button>
							{/each}
		...
		<div class="rounded-[24px] border border-border bg-surface p-7">
			<div class="mb-5 flex flex-wrap items-baseline gap-3.5">
				<span class="rounded-full bg-primary px-2.5 py-1 font-mono text-xs font-bold text-text">E</span>
				<span class="text-xl font-black tracking-[-0.02em]">The desk</span>
				<span class="text-[13px] text-text-muted">Pick the token, then set how hard you back it. Conviction weights the sector's score.</span>
			</div>
			<div class="grid grid-cols-5 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1">
				{#each SECTORS as s, i (s.name)}
					{@const lvl = faders[i]}
					<div class="rounded-2xl border border-border p-4.5">
						<div class="mb-4 flex items-center gap-2.5">
							<div class="grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-black text-text" style="background:{s.color}">
								{s.tokens[0].t.charAt(0)}
							</div>
							<div class="min-w-0">
								<div class="truncate text-sm font-black">{s.tokens[0].t}</div>
								<div class="text-[10px] font-extrabold tracking-[0.08em] uppercase" style="color:{s.color}">{s.name}</div>
							</div>
						</div>
						...
						<span class="ml-1.5 text-[10px] font-extrabold tracking-[0.08em] text-text-muted uppercase">Conviction</span>
					</div>
				{/each}
			</div>
