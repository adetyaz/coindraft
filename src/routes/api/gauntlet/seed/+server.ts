...
export async function POST() {
	...
	if (existing.length > 0) {
		return json({ message: 'Questions already seeded for today' });
	}
	...
	await db.insert(gauntletQuestions).values(questions);

	return json({ message: `Seeded ${questions.length} questions for ${today}` });
}
