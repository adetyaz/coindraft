<script lang="ts">
	...
	function pctClass(value: number) {
		return value >= 0 ? 'text-[#0f6e56] font-medium' : 'text-[#993c1d] font-medium';
	}
</script>

<div class="flex flex-col gap-3">
	<header class="rounded-xl border border-black/10 bg-white px-3 py-2.5">
		<a class="text-[13px] text-[#5d5d6b] no-underline" href="/dashboard">← Exit Result</a>
	</header>
	...
	<section
		class="flex flex-col gap-3.5 rounded-xl bg-[#0f6e56] p-4 text-white"
	...
	<section class="rounded-xl border border-black/10 bg-white px-3.5 py-3">
		<div class="mb-2 flex items-center justify-between">
			<h3 class="text-[11px] font-medium text-[#888780] uppercase">Picks Breakdown</h3>
			<small class="text-[11px] text-[#888780]">Daily Contest</small>
		</div>
		<table class="w-full border-collapse text-xs max-[760px]:block">
			<thead>
				<tr>
					<th
						class="border-b border-[#ececec] px-1.5 py-2.5 text-left text-[10px] text-[#888780] uppercase max-[760px]:hidden"
						>Sector</th
					>
					<th
						class="border-b border-[#ececec] px-1.5 py-2.5 text-left text-[10px] text-[#888780] uppercase max-[760px]:hidden"
			...
			<tbody class="max-[760px]:block">
				{#each breakdown as row, i (`${row.sector}-${row.pick}-${i}`)}
					<tr
						class="max-[760px]:mb-2 max-[760px]:block max-[760px]:rounded-lg max-[760px]:border max-[760px]:border-[#ececec] max-[760px]:p-2"
					>
						<td
							class="border-b border-[#ececec] px-1.5 py-2.5 text-left max-[760px]:block max-[760px]:border-none max-[760px]:px-0 max-[760px]:py-0.5"
							><span
								class="rounded-full bg-[#eeedfe] px-2 py-0.5 text-[10px] font-medium text-[#534ab7]"
								>{row.sector}</span
							></td
						>
						<td
						...
						>
						<td
							class={`border-b border-[#ececec] px-1.5 py-2.5 text-left max-[760px]:block max-[760px]:border-none max-[760px]:px-0 max-[760px]:py-0.5 ${pctClass(row.pct)}`}
	...
	<section class="rounded-xl border border-black/10 bg-white px-3.5 py-3">
		<div class="mb-2 flex items-center justify-between">
			<h3 class="text-[11px] font-medium text-[#888780] uppercase">AI Match Breakdown</h3>
			<small class="text-[10px] text-[#888780]">Powered by SoSoValue + Groq</small>
		</div>
		{#if aiLoading}
			<div class="mt-2 h-4 w-3/4 animate-pulse rounded bg-[#f0eff8]"></div>
			<div class="mt-1.5 h-4 w-full animate-pulse rounded bg-[#f0eff8]"></div>
			<div class="mt-1.5 h-4 w-2/3 animate-pulse rounded bg-[#f0eff8]"></div>
		{:else}
			<p class="mt-2 text-sm leading-[1.6] text-[#474553]">
				{aiBreakdown ||
	...
	<section class="grid grid-cols-2 gap-2.5 max-[760px]:grid-cols-1">
		<a
			href="/dashboard"
			class="grid h-11 place-items-center rounded-lg border border-[#c8c4d5] bg-white text-[13px] font-medium text-[#5d5d6b] no-underline"
			>View League Standings</a
		>
		<a
			href="/draft"
			class="grid h-11 place-items-center rounded-lg bg-[#534ab7] text-[13px] font-medium text-[#eeedfe] no-underline"
			>Draft Again</a
		>
	</section>
</div>
